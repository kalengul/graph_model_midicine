...
from constants import nlp, print_list
import time
...
def find_mechanism_connections(doc, ner_tokens):
	...
	for mechanism in mechanism_tokens:
		...
		def get_conj_tokens(doc, filter_func):
			return [(token, token.i) for token in doc for base_token, _ in filter_func if token.dep_ == "conj" and token.head == base_token]
