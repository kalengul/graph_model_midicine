    ...
    def post(self, request):
        ...
        for se in data["side_effects"]:
            result["side_effects"][0]["effects"].append({
                self.EFFECT_NAME: se,
                "rank": round(data["side_effects"][se]["probability"], 2),
            })
...
#         for drug in drugs:
#             single_prob_data, single_drug_states_input_data, \
#                 single_drugs_for_output, combination_description = \
#                 load_combined_data(
#                     graph_data=graph,
#                     prob_file=graph_storage.probability_path,
#                     drug_states_input=self._get_bin_ids([drug])
...
#             single_network = build_network(graph, single_prob_data)
#             single_probs  = calculate_probabilities(single_network,
#                                                     'calculation_trace.txt')

#             individual_data  = get_result(
#                 single_probs,
#                 graph,
#                 single_drug_states_input_data,
#                 single_drugs_for_output,
#                 combination_description)
...
#                     drug_effects.append({
#                         self.EFFECT_NAME: se_name,
#                         self.RANK: se_data["probability"]
#                     })

#             drug_effects.sort(key=lambda x: x[self.RANK], reverse=True)

#             if gender:
#                 drug_effects = self._exclude_by_gender(drug_effects,
#                                                        gender,
#                                                        GENDER_SIDE_EFFECT)
