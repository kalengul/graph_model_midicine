...
class BayesianNode:
    def __init__(self, node_id, name, parents, prob_data, label=""):
        self.id = node_id
        self.name = name
        ...
        # Определяем тип зависимости от родителей на основе наличия CPT
        # Если у узла есть родители, но CPT пустой - узел независимый
        if parents and not self.prob_table:
            self.dependent = False
        else:
            # Зависимый узел (имеет CPT) или корневой узел
            self.dependent = True

    def _parse_probabilities(self, prob_data):
        parsed = {}
        
        # Если prob_data пустой словарь (для независимых узлов)
        if not prob_data:
            return parsed
            
        for k, v in prob_data.items():
            if isinstance(k, str):
                # Обработка строковых ключей
                if k == "":
                    key = ()
                else:
                ...
                # Для независимых узлов с родителями prob_data может быть пустым словарем
                key = ()
            parsed[key] = float(v)
        return parsed
...
def create_base_probabilities(graph_data, output_file='probabilities.json'):
    ...
    for node in graph_data['nodes']:
        ...
        if node_id in zero_nodes:
            if not parents:
                probabilities[node_id] = {"": 0.0}
            else:
                # Для зависимых узлов создаем CPT
                combs = product([0], repeat=len(parents))  # Все комбинации нулей
                probabilities[node_id] = {','.join(map(str, c)): 0.0 for c in combs}
        else:
            # Случайные вероятности для остальных
            if not parents:
                probabilities[node_id] = {"": random.uniform(0.00001, 0.999)}
            else:
                # Для зависимых узлов создаем полную CPT
                combs = product([0, 1], repeat=len(parents))
                probabilities[node_id] = {','.join(map(str, c)): random.uniform(0.0000001, 0.999) for c in combs}
    ...
    for drug_id, state in drug_states_input.items():
        ...
        if drug_id and drug_id in probabilities:
            ...
            if "" in probabilities[drug_id]:
                ...
                # Определяем, является ли узел зависимым (есть ли CPT)
                parents = parent_map.get(drug_id, [])
                if probabilities[drug_id]:
                    # Для зависимых узлов применяем состояние ко всем комбинациям
                    probabilities[drug_id] = {k: float(state) for k in probabilities[drug_id]}
                else:
                    # Для независимых узлов с родителями или узлов без CPT
                    probabilities[drug_id] = {"": float(state)}
...
def calculate_probabilities(network, calc_trace_path):
    ...
    with open(calc_trace_path, 'w', encoding='utf-8') as f:
        ...
        for node_id in sorted_nodes:
            ...
            f.write(f"Тип: {node.label}\n")
            f.write(f"Зависимость: {'Зависимый' if node.dependent else 'Независимый'}\n")
            f.write(f"Родители: {node.parents or 'нет'}\n")
            f.write(f"Количество родителей: {len(node.parents)}\n")
            f.write(f"CPT размер: {len(node.prob_table)}\n")

            # Проверяем, является ли узел независимым
            if not node.dependent and node.parents:
                # Независимый узел с родителями: P(Node=1) = Π P(Parent_i=1)
                f.write("Расчет для независимого узла: P(Node=1) = Π P(Parent_i=1)\n")
                prob = 1.0
                for i, parent_id in enumerate(node.parents, 1):
                    ...
                    f.write(f"  Родитель {i}: {parent.name} (ID {parent_id})\n")
                    f.write(f"  P({parent.name}=1) = {parent_prob:.4f}\n")
                    f.write(f"  Текущее произведение: {prob:.4f} * {parent_prob:.4f} = {prob * parent_prob:.4f}\n")
                    prob *= parent_prob
                
                probabilities[node_id] = prob
                f.write(f"Итоговая вероятность: {prob:.4f}\n")
                f.write("="*50 + "\n\n")
                continue
            
            # Зависимые узлы и независимые узлы без родителей
            if not node.parents:
                # Узел без родителей
                prob = node.get_probability(())
                if prob == 0.0 and node.prob_table:
                    # Если в CPT есть данные, но не для пустого кортежа
                    for key, value in node.prob_table.items():
                        prob = value
                        break
                probabilities[node_id] = prob
            ...
            # Зависимый узел с родителями
            f.write("Расчет для зависимого узла (используется CPT):\n")
            
            if not node.prob_table:
                f.write("ПРЕДУПРЕЖДЕНИЕ: У зависимого узла пустая CPT! Используется вероятность по умолчанию 0.0\n")
                probabilities[node_id] = 0.0
                f.write("="*50 + "\n\n")
                continue
            ...
            for i, (comb, info_node) in enumerate(normalized_probs.items(), 1):
                ...
                f.write(f"Накопленная сумма (свертка): {total:.4f}\n")
...
def get_conditional_probability(network, node_id, parent_states):
    ...
    # Для независимых узлов с родителями возвращаем 0.0, так как у них нет CPT
    if not node.dependent and node.parents:
        return 0.0
...
def debug_print_network(network):
    """Вспомогательная функция для отладки: выводит информацию о сети"""
    print("\n" + "="*60)
    ...
    for node_id, node in network.items():
        print(f"\nУзел: {node.name} (ID: {node_id})")
        print(f"  Тип: {node.label}")
        print(f"  Зависимость: {'Зависимый' if node.dependent else 'Независимый'}")
        print(f"  Родители: {node.parents}")
        print(f"  Количество родителей: {len(node.parents)}")
        print(f"  CPT размер: {len(node.prob_table)}")
    ...
    print(f"  Зависимых узлов: {dependent_nodes}")
    print(f"  Независимых узлов: {independent_nodes}")
    print(f"  Корневых узлов: {root_nodes}")
    print("="*60)


if __name__ == "__main__":
    # Загрузка графа
    with open('bayes_exploiatation\\data\\graphs_10_6.json', encoding='utf-8') as f:
        graph = json.load(f)

    with open('bayes_exploiatation\\data\\drug_states_10_5.json', encoding='utf-8') as f:
    ...
    prob_data, drug_states_input_data, drugs_for_output, combination_description = load_combined_data(
        graph_data=graph,
        prob_file='bayes_exploiatation\\data\\probabilities_opt_pytorch.json', 
        drug_states_input=drug_states_input
    )

    print(f"Загружено {len(prob_data)} узлов с вероятностями")
    print(f"Количество препаратов в комбинации: {len(drugs_for_output)}")

    # Построение сети с новыми параметрами
    network = build_network(graph, prob_data)
    
    # Отладочная информация о сети
    debug_print_network(network)

    # Перерасчет вероятностей
    final_probs = calculate_probabilities(
        network, 
        "bayes_exploiatation\\data\\opt_calc.txt"
    )
    ...
    print("\nРасчеты успешно завершены. Результаты сохранены в results.json")
    
    # Дополнительная информация о независимых узлах
    independent_nodes = [(node.name, len(node.parents)) 
                        for node in network.values() 
                        if not node.dependent and node.parents]
    if independent_nodes:
        print(f"\nНезависимые узлы (вероятность = произведение вероятностей родителей):")
        for node_name, parent_count in independent_nodes:
            print(f"  - {node_name} (родителей: {parent_count})")
    
    # Информация о побочных эффектах
    side_effect_nodes = [node for node in network.values() 
                        if node.label in ['side_effect', 'side_e', 'effect']]
    print(f"\nНайдено побочных эффектов: {len(side_effect_nodes)}")