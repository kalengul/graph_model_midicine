...
if __name__ == "__main__":
    # Загрузка графа
    with open('bayes_exploiatation\\data\\graphs_10_4.json', encoding='utf-8') as f: # Предполагается, что это ваш файл графа
        graph = json.load(f)

    with open('bayes_exploiatation\\data\\drug_states.json', encoding='utf-8') as f: # Предполагается, что это ваш файл графа
    ...
    prob_data, drug_states_input_data, drugs_for_output, combination_description = load_combined_data(
        graph_data=graph,
        prob_file='bayes_exploiatation\\data\\probabilities_opt_pytorch_067.json',  # Теперь этот файл - источник общих вероятностей
    ...
    final_probs = calculate_probabilities(network, "bayes_exploiatation\\data\\opt_calc.txt")
...
#             for comb, p in node.prob_table.items():
#                 # normalized_probs[comb] = (p, p / total_conditional if total_conditional != 0 else 0.0, total_conditional)
#                 normalized_probs[comb] = (p, p if total_conditional != 0
#                                           else 0.0, total_conditional)
