...
class JSONLoader:
    ...
    def load_json_Medscape(self, BASE_DIR):
        """Загрузка данных."""
        ...
        json_folder_medscape_en = os.path.join(BASE_DIR, 'testmedscape_en')
        json_folder_medscape_ru = os.path.join(BASE_DIR, 'testmedscape_ru')
        s = 'START'
            ...
            if file_name.endswith('.json'):  # если файл имеет расширение .json
                ...
                with open(os.path.join(json_folder_medscape_en, file_name),
                    ...
                    while i < max_len:
                        ...
                        type_drug_obj, _ = TypeDrugsMedScape.objects.get_or_create(Type_en=type_drug_en,
                    ...
                    group_type_obj, _ = NameDrugsMedScape.objects.get_or_create(Name_en=group_type_en,
                                                                                Name_ru=group_type_ru)

                    i_arr_drug_obj = 0
                    while i_arr_drug_obj < len(arr_drug_obj):
                        group_type_obj.GroupType.add(arr_drug_obj[i_arr_drug_obj])
