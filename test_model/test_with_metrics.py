...
'''
test_sentences = ["Hawking was a theoretical physicist.", "Apple is looking at buying U.K. startup for $1 billion."]
test_labels = [
    ['B-PER', 'O', 'O', 'O', 'O', 'O'],
    ['B-ORG', 'O', 'O', 'O', 'B-LOC', 'O', 'O', 'O', 'O']
]
'''
...
test_labels = list()
test_tokens = list()
with open('test_mark_text.json', 'r', encoding='utf-8') as file:
    data = json.load(file)
    for item in data:
...
for model_name, model_name2 in zip(mark_sent_v2_rubert_base_cased_model_names, mark_sent_v2_RuBioBERT_model_names):
    ...
    plt.bar(metrics, values, color=['blue', 'orange', 'green'])
    plt.ylim(0, 1)
    plt.title('NER Metrics for RuBioBERT Model')
    plt.xlabel('Metrics')
    plt.ylabel('Score')
    ...
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate")
    plt.title("ROC-AUC for NER with RuBioBERT")
    plt.legend(loc="lower right")
    ...
    plt.xlabel("Recall")
    plt.ylabel("Precision")
    plt.title("Precision-Recall curve for NER with RuBioBERT")
    plt.legend(loc="lower left")
