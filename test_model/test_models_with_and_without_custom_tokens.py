...
annotation = r'content\mark_sent_2.json'
...
rubert_model_names = ['saved_model_mark_sent_2_rubert-base-cased_5',
                    'saved_model_mark_sent_2_rubert-base-cased_6',]


rubert_model_with_custom_tokens_names = ['saved_model_with_custom_tokenizer_16_mark_sent_2_rubert-base-cased_5',
                                        'saved_model_with_custom_tokenizer_16_mark_sent_2_rubert-base-cased_6',]
...
test_labels = list()
test_tokens = list()
with open(r'content\test_mark_text.json', 'r', encoding='utf-8') as file:
    data = json.load(file)
    for item in data:
...
for model_name, model_name2 in zip(rubert_model_names, rubert_model_with_custom_tokens_names):
    ...
    # Сохранение графика
    #plt.savefig(f'ner_metrics_{model_name}.png')
    ...
    # Сохранение графика
    #plt.savefig(f'ner_roc_auc_{model_name}.png')
    ...
    # Сохранение графика
    #plt.savefig(f'ner_precision_recall_{model_name}.png')
    ...
    # Подробный отчет
    #print("\nПодробный отчет:\n")
    #print(classification_report(test_labels_cleared, pred_labels_cleared))
    ...
    model = AutoModelForTokenClassification.from_pretrained(model_name2, num_labels=len(label_list), trust_remote_code=True)
    ...
    ws.cell(row=row_number, column=COLUMN_MODEL_NAME, value=model_name2)
    ws.cell(row=row_number, column=COLUMN_ACCURACY, value=accuracy)
    ws.cell(row=row_number, column=COLUMN_PRECISION, value=precision)
    ws.cell(row=row_number, column=COLUMN_RECALL, value=recall)
    ws.cell(row=row_number, column=COLUMN_F1, value=f1)
    ...
    plt.bar(metrics, values, color=['blue', 'orange', 'green'])
    plt.ylim(0, 1)
    plt.title('NER Metrics for ruBERT Model with custom tokenizer')
    plt.xlabel('Metrics')
    plt.ylabel('Score')

    # Сохранение графика
    plt.savefig(f'ner_metrics_{model_name2}.png')
    ...
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate")
    plt.title("ROC-AUC for NER with ruBERT Model with custom tokenizer")
    plt.legend(loc="lower right")

    # Сохранение графика
    plt.savefig(f'ner_roc_auc_{model_name2}.png')
    ...
    plt.xlabel("Recall")
    plt.ylabel("Precision")
    plt.title("Precision-Recall curve for NER ruBERT Model with custom tokenizer")
    plt.legend(loc="lower left")

    # Сохранение графика
    plt.savefig(f'ner_precision_recall_{model_name2}.png')

    
    

#wb.save(f'Сравнение оценок качества работы моделей NER.xlsx')
...
#row_number = 1
'''
for model_name, model_name2 in zip(mark_sent_v2_rut5_base_model_names, mark_sent_v2_rut5_base_multitask_model_names):
    ...
    for test_sentence in test_sentences:
        ...
        tokens, predicted_token_class = merge_T5_tokens(tokens, predicted_token_class)
        tokens_list.append(tokens)
        #predicted_token_class = predicted_token_class[1:-1]
        pred_labels.append(predicted_token_class)
    ...
    plt.bar(metrics, values, color=['blue', 'orange', 'green'])
    plt.ylim(0, 1)
    plt.title('NER Metrics for ruT5 Model')
    plt.xlabel('Metrics')
    plt.ylabel('Score')
    ...
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate")
    plt.title("ROC-AUC for NER with ruT5")
    plt.legend(loc="lower right")
    ...
    plt.xlabel("Recall")
    plt.ylabel("Precision")
    plt.title("Precision-Recall curve for NER with ruT5")
    plt.legend(loc="lower left")
    ...
    plt.bar(metrics, values, color=['blue', 'orange', 'green'])
    plt.ylim(0, 1)
    plt.title('NER Metrics for ruT5 multitask Model')
    plt.xlabel('Metrics')
    plt.ylabel('Score')
    ...
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate")
    plt.title("ROC-AUC for NER with ruT5 multitask")
    plt.legend(loc="lower right")
    ...
    plt.xlabel("Recall")
    plt.ylabel("Precision")
    plt.title("Precision-Recall curve for NER with ruT5 multitask")
    plt.legend(loc="lower left")
...
SAVING_EXCEL_FILE = ('Сравнение оценок качества работы моделей ruBERT ' 
                     'для NER с соединёнными и пользовательскими токенами.xlsx'
                    )
wb.save(SAVING_EXCEL_FILE)
