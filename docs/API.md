...
# API для удаления побочных эффектов, ЛС и групп ЛС
## Назначение
Удаление побочных эффектов, ЛС и групп ЛС
## URL
```DELETE api/v1/delete/?drug_id=[id]&?se_id=[id]&?dg_id=[id]```
## Аутентификация
Требуемый статус: `admin`

## Вход
На вход в теле запроса получает id побочки, id ЛС или id группы ЛС, если никакой из параметов не отправляется, то выдается ошибка 400<br/>
...
### Пример входныйх данных
Для удаления ЛС ```PUT api/v1/delete/?drug_id=1``` <br/>
Для удаления группы ЛС ```PUT api/v1/delete/?dg_id=1``` <br/>
Для удаления побочного эффекта ```PUT api/v1/delete/?se_id=1``` <br/>
